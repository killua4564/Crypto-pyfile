...
xor: callable = lambda x, y: bytes([i ^ j for i, j in zip(x, y)])
block: callable = lambda x, n: [x[i:i+n] for i in range(0, len(x), n)]


def CFB(segment_size: int, plaintext: bytes) -> bytes:
...
def ECB(segment_size: int, plaintext: bytes) -> bytes:
    assert segment_size % 8 == 0
    assert len(plaintext) % (segment_size // 8) == 0
    segment_size //= 8
